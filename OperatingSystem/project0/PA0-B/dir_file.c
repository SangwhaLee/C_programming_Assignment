...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token) {
    /* Fill this function */
    char** arr_temp=token_list;
    directory_t* dir_temp =root_dir;
    int index=0;
    directory_t* new_dir;
    file_t* new_file;
    while(1){
        bool do_create = false;
        if(index<num_token-1){
            new_dir=find_create_dir(dir_temp,*(token_list+index), &do_create);
                if(do_create==true){
                    dir_temp->dir_list[dir_temp->num_dir]=new_dir;
                    dir_temp->num_dir++;
                }
                dir_temp = new_dir;
                index++;
            ...
            new_file=find_create_file(dir_temp, *(token_list+index),&do_create);
            if(do_create==true){
                dir_temp->file_list[dir_temp->num_file] = new_file;
                dir_temp->num_file++;
            }
            return;
        }
